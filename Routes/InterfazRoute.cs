    ...
    public class InterfazRoute : BaseRoute {
        ...
        public async Task<IActionResult> RegistrarCredenciales([FromBody] Credenciales modeloCredenciales) {
            try {
                ...
                if (modeloCredenciales.role == null) return BadRequest("role no puede ser null");
                if (modeloCredenciales.token == null) return BadRequest("token no puede ser null");

                var funsiona = await loginDAL.InsertarCredenciales(modeloCredenciales.name, modeloCredenciales.pwd, modeloCredenciales.role, modeloCredenciales.token);
