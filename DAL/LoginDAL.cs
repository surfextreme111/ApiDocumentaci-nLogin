...
namespace DocumentationLogin3.DAL {
    public class LoginDAL {
        public async Task<string> HaceLogin(string usuario, string contraseña) {
            try {
                using (var connection = new NpgsqlConnection("Server=192.168.11.51;User ID=postgres;Password=ui+9AjMfj+?Vh;Database=DocumentationSandBox;Port=5433;CommandTimeout=300;IncludeErrorDetail=true;")) {
        ...
        public async Task<string> InsertarCredenciales(string name, string pwd, string role, string token) {
            try {
                var connectionString = "Server=192.168.11.51;User ID=postgres;Password=ui+9AjMfj+?Vh;Database=DocumentationSandBox;Port=5433;CommandTimeout=300;IncludeErrorDetail=true;";
                var insertQuery = "INSERT INTO docu.users (uname, pwd, userrole, token) VALUES (@name, @pwd, @role, @token)";
        ...
        public async Task<List<Credenciales>> RecuperarUsuarios() {
            ...
            try {
                using (var conn = new NpgsqlConnection("Server=192.168.11.51;User ID=postgres;Password=ui+9AjMfj+?Vh;Database=DocumentationSandBox;Port=5433;CommandTimeout=300;IncludeErrorDetail=true;")) {
                    await conn.OpenAsync();
                    using (var cmd = new NpgsqlCommand($"SELECT id, uname, pwd, userrole, token FROM docu.users", conn)) {
